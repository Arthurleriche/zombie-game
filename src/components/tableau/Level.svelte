<script>
  ...
  const updateMoveHero = () => {
    ...
    switch (true) {
      ...
      case left:
        $direction = 'step-left';
        if( collision &&  $ligAlien -2 <= $ligHero  && $ligHero <=  $ligAlien + 2 && $colHero <= $colAlien + 2 && $colHero > $colAlien){
          $direction = 'left';
        } else {
...
<div class="level">
  <div class="debug-mode">
    <Debug>
      <div class="flex">
        <div class="HeroPos h-auto w-40 border border-black bg-red-400"> <span class='bg-red-600'>Hero</span> <br> x: {xHero} <br> y: {yHero}  </div>
        <div class="AlienPos h-auto w-40 border border-black bg-orange-400"> <span class='bg-orange-600'>Alien</span> <br> x: {xAlien} <br> y: {yAlien}</div>
      </div>
